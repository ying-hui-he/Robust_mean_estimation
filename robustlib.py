...
class Top_K_Filtered(FilterAlgs):
    ...
    def trim_data(self, S, top_indices):
        ...
        S_new = np.zeros((sample_size, k))
        for i in range(sample_size):
            new_vec = S[i][top_indices]
            S_new[i] = new_vec.copy()
        self.params.d = k
        return S_new
...
class RSPCAb(FilterAlgs):
    ...
    def guess_restricted_cov(self, S_cov, u, indices, v_prev):
        ...
        Q2 = np.array([np.tile(Q, (len(Q), 1)), np.repeat(
            Q, len(Q), 0)]).transpose([1, 0, 2])

        Cov = np.outer(v_prev, v_prev)

        T1 = (Cov[Q2[:, 0, 0], Q2[:, 1, 1]] *
              Cov[Q2[:, 0, 1], Q2[:, 1, 0]]).reshape(k**2, k**2)
        vecCov = (np.identity(d) + np.outer(v_prev, v_prev)
                  ).reshape((d**2,))[u]
    ...
    def boot_iteration(self, S, indicator, v_prev, thresh):
        ...
        while True:
            ...
            if ev < thresh:
                ...
                idx = self.drop_points(
                    S, indicator, x, self.tail_rspca, self.do_plot_rspca, self.figure_no)[0]
                self.figure_no += 1

                if self.verbose:
                    bad_filtered = np.sum(indicator) - np.sum(indicator[idx])
                    print(
                        f"Filtered out {l - len(idx)}/{l}, {bad_filtered} false ({bad_filtered / (l - len(idx)):0.2f} vs {self.fdr})")
    ...
    def alg(self, S, indicator, tv=0):
        ...
        for i in range(biter):
            ...
            if self.verbose:
                print(f"loss after {i+1} iteration!",
                      LA.norm(np.outer(v_prev, v_prev) - np.outer(tv, tv)))
        return v_prev
...
class RSPCAdense(FilterAlgs):
    ...
    def tail_rspcaDense(self, T):
        ...
        # + (eps**2)/(np.log(d*np.log(m*d/tau))*T**2))
    ...
    def alg(self, S, indicator):
        ...
        while True:
            ...
            if ev > dsort[int(0.25*len(dots))] and ev < dsort[int(0.75*len(dots))]:
                ...
                self.stddev = (
                    1/1.25)*np.abs(dsort[int(0.75*len(dots))]-dsort[int(0.25*len(dots))])
                ...
                idx = self.drop_points(
                    S, indicator, x, self.tail_rspcaDense, self.do_plot_rspca, self.figure_no)[0]
                self.figure_no += 1
        ...
        for xvar in xs:
            ...
            if y_is_m == False:
                ...
                for f in self.keys:
                    ...
                    if relative == True:
                        results.setdefault(f.__name__, []).append(self.loss(func.alg(
                            S_copy, indicator_copy), tm)/self.loss(O.alg(S_copy, indicator_copy), tm))
                    else:
                        results.setdefault(f.__name__, []).append(
                            self.loss(func.alg(S_copy, indicator_copy), tm))
...
class plot_data(RunCollection):
    ...
    def plot_xloss(self, outputfilename, runs, xvar_name, bounds, title, xlabel, ylabel, ylims, y_is_m=False, relative=False, explicit_xs=False, xs=[], fsize=10, fpad=10, figsize=(1, 1), fontname='Arial'):
        ...
        cols = {'RSPCAdense': 'k', 'RSPCAb': 'b', 'RME_sp': 'b', 'RME_sp_L': 'g', 'RME': 'r', 'ransacGaussianMean': 'y',
                'NP_sp': 'k', 'Oracle': 'c', 'Top_K': 'darkseagreen', 'Top_K_Filtered': 'palevioletred'}
